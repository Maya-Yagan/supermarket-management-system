...
public class OrderManagementController implements Initializable {
    ...
    private void setupEventHandlers(){
        ordersButton.setOnAction(event -> FormHelper.openForm(
                "/fxml/OrdersDisplay.fxml",
                (OrdersDisplayController controller) -> {
    ...
    private void setupDynamicLayoutAdjustment(){
        categoryMenuButton.widthProperty().addListener((observableList, oldValue, newValue) -> {
            ...
            ordersButton.setLayoutX(categoryMenuButton.getLayoutX() + newValue.doubleValue() + 140);
        });
        supplierMenuButton.widthProperty().addListener((obs, oldVal, newVal) -> {
            ordersButton.setLayoutX(supplierMenuButton.getLayoutX() + newVal.doubleValue() + 10);
        });
    }
